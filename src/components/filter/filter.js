...
const Filter = ({ categories, cType, title, isMovieCard }) => {
  ...
  if (showFadedContainer) {
    if (isMovieCard) {
      ...
      content = <div
        className='faded-container col-12'
        style={{ zIndex: 1, height: "320px", backgroundImage: "rgba(3, 37, 65, 0.75) 0%)" }}>
      </div>;
    }
  }
  return (
    <div className={`container filter mt-5 ${!isMovieCard ? "trailer_bg" : (cType === "trending" ? "trending_bg" : "")}`}>
      <div className={`row filter-bottom__wrapper filter-bottom__wrapper-${cType}`}>
        <FilterTop
          cType={cType} title={title}
          categories={categories}
          isMovieCard={isMovieCard}
          storeCardDataHandler={storeCardDataHandler}
          showFadedContainerHandler={showFadedContainerHandler} />
        {content}
      </div>
